...
async function updateLesson(object) {
    ...
    try {
        const lesson_find = await client.query(object.updateAll ? `select * from schedule where "sequenceId" = (select "sequenceId" from schedule where id = $1)` : `select * from schedule where id = $1`, [object.lessonId])
        ...
        if (!object.updateAll) {
            ...
            const edit = lesson_find.rows.find(r => r.id == object.lessonId)
            let [offS, offE] = [
                startDate.valueOf() - (new Date(edit.startDate)).valueOf(),
                endDate.valueOf() - (new Date(edit.endDate)).valueOf(),
            ]
            for (const row of lesson_find.rows) {
                let [s,e] = [(new Date(row.startDate)).valueOf() + offS, (new Date(row.endDate)).valueOf() + offE]
                const lesson = await client.query(`update schedule set "startDate" = $1, "endDate" = $2 where id = $3 returning *`, [new Date(s), new Date(e), row.id])
                if (lesson.rowCount === 0) {
                    data.message = "Занятие не обновлено"
                    data.statusCode = 500
                    log(data.message)
            ...
            data.lessons = lesson_find.rows.length
        }
...
async function deleteLesson(object) {
    ...
    try {
        const del_lesson = object.deleteAll ?
            await client.query(`delete from schedule where "sequenceId" = (select "sequenceId" from schedule where id = $1)`, [object.lessonId]) :
            await client.query(`delete from schedule where id = $1`, [object.lessonId])
